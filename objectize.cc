...
EMSCRIPTEN_KEEPALIVE void addPlaneGeometryPhysics(PScene *scene, float *position, float *quaternion, unsigned int id, PxMaterial *material, unsigned int dynamic, int groupId) {
  scene->addPlaneGeometry(position, quaternion, id, material, dynamic, groupId);
}
